...
- 自定义扩展：已实现三个真实工具——章节查询、随机抽题、客观题判分（见 `tools/`），并已接入助教模型。
- 系统测试：15 项必测已执行并记录（见 [tests/测试题集.md](tests/测试题集.md)）。
...
- 团队成员（4 人分工）：
  ...
  - 自定义扩展 & 系统测试：孟子豪、尹志学
  - 工程/集成/部署机维护：孟子豪
- 共享访问地址：`http://localhost:8080`（本机使用；组员远程访问暂未开放，后续可按校园网 IP 或 Tailscale 开放）
- GitHub 仓库链接：https://github.com/804216/course-ai-assistant
...
- [x] M4 测试：15 项必测已执行并记录（见 `tests/测试题集.md`）；优化前后对比暂未做
- [x] 自定义扩展：章节查询、随机抽题、客观题判分三个工具已实现并接入
- [x] M5 提交物整理：搭建记录/测试记录与项目总结报告已完成（见 [docs/项目总结报告.md](docs/项目总结报告.md)）
