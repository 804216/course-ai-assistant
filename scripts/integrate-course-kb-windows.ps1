...
param(
    ...
    [string]$BaseModelId = 'deepseek-v4-flash'
)
...
$installer = Join-Path $projectRoot 'tools\course_kb\integrate_open_webui.py'
...
& $venvPython $installer --base-url $BaseUrl --base-model-id $BaseModelId
if ($LASTEXITCODE -ne 0) {
    throw "Course knowledge integration failed with exit code $LASTEXITCODE"
}
